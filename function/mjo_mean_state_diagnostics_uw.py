...
def remove_anncycle_new(x,dt): #x is one dimension, dt represents how many days in 1 time interval (3hourly-->dt=1/8, daily-->dt=1)
    ...
    return xp, xcycle
'''
# This is the correct version of it. (2025.2.12)
# Update: add a function to output nan if the entire timeseries is nan
def remove_anncycle_new(x,dt): #x is one dimension, dt represents how many days in 1 time interval (3hourly-->dt=1/8, daily-->dt=1)
    mmax = 6 # remove mean and first 3 harmonics
    n = np.size(x)
    if np.sum(np.isnan(x)==0)==0:# or xm!=np.nanmean(x):
        #print('Caution: the entire timeseries is nan, and thus the anomaly is also nan!')
        xp = np.empty([n])
        xp[:] = np.nan
        xcycle = np.empty([n])
        xcycle[:] = np.nan
